...
//connecting to database for data retrieval, storing and updating
include("includes/db.php");

//variable to store processing respomse messages
$message = "";
$message_type = "";

//statement to run when reset password was clicked
if(isset($_POST['reset_password'])){

    //storing users entered data 
    $email = trim($_POST['email']);
    $sa_id = trim($_POST['sa_id']);
    $new_password = trim($_POST['new_password']);
    $confirm_password = trim($_POST['confirm_password']);

    //checking if user left any empty input fields 
    if(empty($email) || empty($sa_id) || empty($new_password) || empty($confirm_password)){

        $message = "Please complete all fields.";
        $message_type = "error-message";

    }elseif($new_password !== $confirm_password){//and checking if password match
        ...
        //getting user information and avoiding SQL inh=jections
