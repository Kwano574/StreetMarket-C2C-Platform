...
// function to protect links that need login
function protectedLink($page, $is_logged_in){

    if($is_logged_in){
        return $page;
    }else{
        return "login.php?redirect=" . urlencode($page);
    }
...
<a href="messages.php" title="Chat">&#x1F4AC; Chat</a>
...
<form class="search-row" action="index.php" method="GET">
...
<a href="index.php" class="view-products-link">Clear Search</a>
...
AND (
product.product_name LIKE '%$search_safe%'
OR product.category LIKE '%$search_safe%'
OR users.full_name LIKE '%$search_safe%'
)
...
<a href="<?php echo htmlspecialchars($details_link); ?>" class="primary-btn">View Product</a>
...
<p>No approved products matched your search.</p>
...
<a href="products.php" class="primary-btn">Browse Available Products</a>

<a href="dashboard.php" class="secondary-btn">Go To Dashboard</a>

<?php }else{ ?>

<a href="register.php" class="primary-btn">Create an Account</a>

<a href="login.php?redirect=<?php echo urlencode('products.php'); ?>" class="secondary-btn">Browse Available Products</a>
...
<a href="<?php echo htmlspecialchars(protectedLink('products.php', $is_logged_in)); ?>" class="view-products-link">View All Products</a>
...
if($category_result && mysqli_num_rows($category_result) > 0){
    ...
    while($catRow = mysqli_fetch_assoc($category_result)){
        ...
        $category_page = "products.php?category=" . urlencode($catNameRaw);
        $category_link = protectedLink($category_page, $is_logged_in);

?>

<a href="<?php echo htmlspecialchars($category_link); ?>" class="category-card">
...
<p>Guests can search and preview approved products before creating an account.</p>
...
<h3>Login To View Details</h3>

<p>Users must login before viewing full product details, buying, messaging or managing a cart.</p>
