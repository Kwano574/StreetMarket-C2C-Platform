...
if(!isset($_SESSION['user_id'])){
    ...
    exit();//exit page if user is not logged in

}

$user_id = $_SESSION['user_id'];//storing logged in user session ID
$message = "";

//statement that runs when user updates their product quantity
...
//statement that runs when user is removing product 
...
//cpunitng how many products seller uploaded
...
$count_listings_result = mysqli_query($conn,$count_listings_query);
$count_listings_data = mysqli_fetch_assoc($count_listings_result);
...
$count_sold_result = mysqli_query($conn,$count_sold_query);

$count_sold_data = mysqli_fetch_assoc($count_sold_result);
...
$earnings_data = mysqli_fetch_assoc( $total_earnings_result);
...
<h2>🏪 My Listings</h2>
<p>Manage your uploaded products, update quantity and monitor buyer activity.</p>
</div>
</div>
</section>
...
if($listings_result && mysqli_num_rows($listings_result) > 0){
    while($listing = mysqli_fetch_assoc($listings_result)){
        ...
        $msg_count_result = mysqli_query( $conn, $msg_count_query);
        $msg_count_data = mysqli_fetch_assoc($msg_count_result);
...
<input  type="hidden"  name="product_id"  value="<?php echo $product_id; ?>">
<button type="submit" name="remove_product"> Remove Listing </button>
</form>
